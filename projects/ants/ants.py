...
class Bee(Insect):
    ...
    def __init__(self, armor, place=None): # 重载构造函数, 主要是增加了scared 和 direction
        self.armor = armor
        self.place = place
        self.scared = False # False时表明该蜜蜂尚未被惊吓, 反之同理
        self.direction = 1
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        onePlace = self.place
        pos = 0
        while onePlace is not hive:
            bee_or_none = random_or_none(onePlace.bees)
            if (bee_or_none is not None) and (pos >= self.min_range) and (pos <= self.max_range):
                return bee_or_none
            else:
                onePlace = onePlace.entrance
                pos += 1
        return None
